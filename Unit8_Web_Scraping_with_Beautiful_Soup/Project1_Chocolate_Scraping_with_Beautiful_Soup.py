...
# How are ratings distributed? 
choc_ratings = soup.find_all(attrs={"class": "Rating"})
ratings = []

for r in range(1, len(choc_ratings)):  # skip header row
  ratings.append(float(choc_ratings[r].get_text()))
...
# Which chocolatier makes the best chocolate?
choc_companies = soup.select(".Company")
companies = []

for r in range(1, len(choc_companies)):  # skip header row
  companies.append(choc_companies[r].get_text())

choc_d = {"Company": companies, "Ratings": ratings}
choc_df = pd.DataFrame.from_dict(choc_d)
...
# Is more cacao better?
coc_percents = soup.select(".CocoaPercent")
cocoa_percent = []

for r in range(1, len(coc_percents)):  # skip header row
    coc_percent = coc_percents[r].get_text()
    cocoa_percent.append(float(coc_percent[:-1]))  # drop '%' sign and convert str to float

choc_d = {"Company": companies, "Ratings": ratings, "CocoaPercentage": cocoa_percent}
choc_df = pd.DataFrame.from_dict(choc_d)

plt.scatter(choc_df["CocoaPercentage"], choc_df["Ratings"], alpha=0.1)
plt.xlabel("Cocoa Percentage")
plt.ylabel("Rating")
plt.title("Effect of Cocoa Percentage on Rating")


z = np.polyfit(choc_df.CocoaPercentage, choc_df.Ratings, 1)  # is there correlation here?
line_function = np.poly1d(z)
